...
# This script is a small modification of DBDA2E-utilities.R that accompanies the book,
...
plotPost = function( paramSampleVec , cenTend=c("mode","median","mean")[1] , 
  ...
  if ( showCurve ) {
    par(xpd=NA)
    histinfo = hist( paramSampleVec , xlab=xlab , yaxt=yaxt , ylab=ylab ,
                     freq=F , border="white" , col="white" ,
                     xlim=xlim , main=main , cex=cex , cex.lab=cex.lab ,
                     breaks=breaks , ... )
    densCurve = density( paramSampleVec , adjust=2 )
    polygon( densCurve$x , densCurve$y , 
  ...
  if ( !is.null( ROPE ) ) {
    ...
    lines( c(ROPE[1],ROPE[1]) , c(0.92*ROPEtextHt,0) , lty="dotted" , lwd=2 ,
           col=ropeCol )
    lines( c(ROPE[2],ROPE[2]) , c(0.92*ROPEtextHt,0) , lty="dotted" , lwd=2 ,
           col=ropeCol)
    ...
    text( max( ROPE[1] , min( ROPE[2] , mo ) ) , ROPEtextHt ,
          labels=bquote( .(round(100*pInROPE,1))*"% in ROPE" ) ,
          adj=c(0.5,0) , cex=1 , col=ropeCol )
